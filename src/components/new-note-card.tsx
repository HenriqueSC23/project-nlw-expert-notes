...
export function NewNoteCard({ onNoteCreated }: NewNoteCard) {
  ...
  return(
    <Dialog.Root>
      <Dialog.DialogTrigger className='rounded-md flex flex-col bg-slate-700 text-left p-5 space-y-3 outline-none hover:ring-2 hover:ring-slate-600 focus-visible:ring-2 focus-visible:ring-lime-400'>
                  ...
                  <form className='flex-1 flex flex-col'>
                    <div className='flex flex-1 flex-col gap-3 p-5'>
                        ...
                        {shouldShowOnboarding ? (
                          <p className='text-sm leading-6 text-slate-400'>
                          Comece <button type='button' onClick={handleStartedRecording} className='font-medium text-lime-400'>gravando uma nota</button> em áudio ou se preferir <button type='button' onClick={handleStartEditor} className='font-medium text-lime-400'>utilize apenas texto.</button>
                        </p>
                        ) : (
                          <textarea
                          autoFocus
